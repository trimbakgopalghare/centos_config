...
**  gcc -D_REENTRANT -o sample sample.c -lmilter -lresolv  -lnsl -lpthread -I/usr/include/mysql -L/usr/lib/mysql -lmysqlclient -lz -lm -L/usr/lib
**  OR
**  gcc -D_REENTRANT -o sample sample.c -lmilter -lresolv  -lnsl -lpthread `mysql_config --include` `mysql_config --libs`

** gcc -D_REENTRANT -o sample sample.c -lmilter -lresolv  -lnsl -lpthread -I/usr/include/mysql -L/usr/lib/mysql -lmysqlclient -lz -lm -L/usr/lib -lmysqlclient_r
...
sfsistat mlfi_envrcpt(SMFICTX *ctx, char **argv)
{
	...
	MLFIPRIV->user = addslashes(MYSQLconn,MLFIPRIV->user,rcptaddr);
